...
function determinPlatform(url){
    if (url.includes("aniworld.to")) {
        ...
        throw new Error("Unsupported website. Please provide an aniworld.to or s.to link.");
    }
}
...
async function getSeasonLinks(seriesURL) {
    ...
    try {
        ...
        if (!seriesResponse.ok) {
            throw new Error(`Failed to fetch ${seriesURL}: ${seriesResponse.status} ${seriesResponse.statusText}`);
        }
        ...
        const potentialSeasonLinks = Array.from(document.querySelectorAll('a[href*="staffel"]'))
            .map(link => {
                const href = link.getAttribute('href');
                if (!href) return null;
                return href.startsWith('http') ? href : originUrl + href.replace(/^\//, '');
            })
            .filter(link => link);
...
async function getSeasonVideoUrls(seasonURL) {
    ...
    try {
        ...
        if (!htmlResponse.ok) {
            throw new Error(`Failed to fetch ${seasonURL}: ${htmlResponse.status} ${htmlResponse.statusText}`);
        }
        const html = await htmlResponse.text();
        const dom = new JSDOM(html);
        const document = dom.window.document;

        const episodeLinks = Array.from(document.querySelectorAll('ul > li > a[href*="episode"]'));
        ...
        const videoURLRegex = /<a\s+[^>]*class="[^"]*watchEpisode[^"]*"[^>]*href="([^"]*)"/i;
        const seasonNumberRegex = /<meta\s+itemprop="seasonNumber"\s+content="([^"]*)"/i;
            ...
            episodeUrls.map(async url => {
                try {
                    ...
                    if (!episodeResponse.ok) {
                        throw new Error(`Failed to fetch ${url}: ${episodeResponse.status} ${episodeResponse.statusText}`);
                    }
